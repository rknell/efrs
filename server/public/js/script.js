(function ($) {
  ...
  var Flatland = {
    ...
    AffixMenu: function () {
      ...
      navMenu += '<a href="index.html"><img src="images/logo-sml.png" alt="Logo" /></a>'
      navMenu += '</div>'
      ...
      $('#navigation').waypoint(function () { $('#navigation_affix').removeClass('show') }, {offset: -120})
      $('#navigation').waypoint(function () { $('#navigation_affix').addClass('show') }, {offset: -121})
    },
    ScrollSpy: function () {
      $('body').scrollspy({
      ...
      $('body').on('activate.bs.scrollspy', function (evt) {
        ...
        gtag('config', 'UA-109135204-1', {
          'page_title': evt.target.innerText,
          'page_path': '/' + evt.target.innerText
        })
    ...
    SmoothScroll: function () {
      $('a.smooth-scroll').on('click', function (event) {
        ...
        if (window.Response.band(768)) { offsetTop = parseInt($($anchor.attr('href')).offset().top - elemHeight, 0) } else { offsetTop = parseInt($($anchor.attr('href')).offset().top, 0) }
        $('html, body').stop().animate({scrollTop: offsetTop}, 1500, 'easeInOutExpo')
        event.preventDefault()
      })
    },
    FitVids: function () { $('body').fitVids() },
    ...
    Carousel: function () {
      ...
      $('.carousel-slider.gallery-slider').each(function () {
        $(this).slick({
          ...
          }, {breakpoint: 400, settings: {arrows: true, dots: false, slidesToShow: 1, slidesToScroll: 1}}]
        })
      })
      ...
      }).on('afterChange', function () { $(window).trigger('resize.px.parallax') })
    },
    Lightbox: function () { $('.fancybox').fancybox({loop: false}) },
    CounterUp: function () { $('.affa-counter > h4 > span').counterUp({delay: 10, time: 3000}) },
    Parallax: function () {
      $(window).resize(function () { setTimeout(function () { $(window).trigger('resize.px.parallax') }, 100) })
      $('#navigation .navbar-toggle').on('click', function () { setTimeout(function () { $(window).trigger('resize.px.parallax') }, 300) })
    },
    ImgHover: function () {
      $('.img-overlay').hover(function () {
        ...
        setTimeout(function () {
          $elem.find('.overlay-masked .overlay-icon').animate({top: '50%', opacity: 1}, 200)
          $elem.find('.overlay-masked p').fadeIn(200)
        }, 100)
      }, function () {
        var $elem = $(this)
        $elem.find('.overlay-masked .overlay-icon').animate({top: '25%', opacity: 0}, 200)
    ...
    Isotope: function () {
      ...
      $(window).load(function () {
        ...
        }).on('arrangeComplete', function () { $(window).trigger('resize.px.parallax') }).on('layoutComplete', function () { $(window).trigger('resize.px.parallax') })
      })
      $filter.find('a').click(function () {
        var selector = $(this).attr('data-filter')
        $filter.find('a').removeClass('active')
        $(this).addClass('active')
        $container.isotope({filter: selector, animationOptions: {animationDuration: 400, queue: false}})
        return false
      })
    },
    Form: function () {
    ...
    Animated: function () {
      $(window).on('load', function () {
        $('img.parallax-slider').imgpreload({all: function () { $('img.parallax-slider').addClass('loaded') }})
        $('.animation, .animation-visible').each(function () {
          var $element = $(this)
          $element.waypoint(function () {
            var delay = 0
            if ($element.attr('data-delay')) delay = parseInt($element.attr('data-delay'), 0)
            if (!$element.hasClass('animated')) { setTimeout(function () { $element.addClass('animated ' + $element.attr('data-animation')) }, delay) }
            delay = 0
          }, {offset: '90%'})
        })
      })
